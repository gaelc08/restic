...
`resticctl report` emails one plain-text table with the latest
snapshot of every configured share - a daily "did everything actually
back up" overview, sent regardless of `RESTIC_NOTIFY_ON`.
...
```
Daily restic report - manny-01 - 2026-09-24 06:00 CEST

Latest snapshot per share:

ID        Time                 Host        Tags                               Paths                     Size
a1b2c3d4  2026-09-24 01:00:05  manny-01                                       /mnt/share-finance        12.345 GiB
e5f6a7b8  2026-09-24 01:00:41  manny-01                                       /mnt/share-hr             4.567 GiB

--
Automatically generated by restic-report.sh on manny-01.
```
...
delivery path as failure alerting (same MTA requirement). The table
looks up each share's latest snapshot ID individually
(`restic snapshots --path <share> --json --latest 1`, needs `jq`), then
renders them all in a single `restic snapshots <id1> <id2> ...` call -
one aligned table in restic's native format, Paths column included -
with the header/footer decoration restic adds around it (separator
rules, "N snapshots", "Timestamps shown in ...") stripped out, since
that's noise in a plain-text email.
